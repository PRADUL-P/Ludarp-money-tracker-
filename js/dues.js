...
(function () {
            ...
            <div class="card dues-person-card ${isCollapsed ? 'collapsed' : ''}" style="margin-bottom:12px; padding:0; overflow:hidden;">
              <div class="dues-person-header" style="padding:12px 14px; margin-bottom:0;" 
                ...
                <div style="display:flex; align-items:center; gap:8px;">
                  ...
                  <button class="btn-small" style="padding:6px; opacity:0.7; display:flex; align-items:center;" title="Copy Summary"
                      onclick="event.stopPropagation(); window.MT.dues.copyPersonSummary('${person.replace(/'/g, "\\'")}')">
                    ...
                    <div class="due-item-left">
                      ...
                      <div class="due-item-info">
                        ...
                        <div class="due-item-meta">
                          <span class="due-item-badge ${due.type === 'i_owe' ? 'badge-iowe' : 'badge-theyowe'}">
                            ${due.type === 'i_owe' ? 'I Owe' : 'They Owe'}
                          </span>
                          <span class="due-meta-dot">•</span>
                    ...
                    <div class="due-item-right">
                      ...
                      ${!due.paid ? `
                        <div class="due-item-actions">
                          ...
                          <button class="due-del-btn" title="Copy Bill" style="display:flex; align-items:center; justify-content:center;"
                            onclick="window.MT.dues.copyDueItem('${due.id}')">
  ...
  function shareDueItem(id) {
    ...
    const text = `*💰 LUDARP Money Tracker Bill*\n\n*Person:* ${d.person}\n*Item:* ${d.description || 'General Due'}\n*Amount:* ${cur}${d.amount.toFixed(2)}\n*Type:* ${typeLabel}\n*Date:* ${fmtDate(d.date)}\n\n_Shared from my Money Tracker_`;
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
  }

  function getFilteredDuesForPerson(person) {
    const all = loadDues();
    let filtered = all.filter(d => (d.person || '').trim().toLowerCase() === person.toLowerCase());

    // Apply active filter (tab)
    if (activeFilter === 'pending') filtered = filtered.filter(d => !d.paid);
    if (activeFilter === 'i_owe') filtered = filtered.filter(d => !d.paid && d.type === 'i_owe');
    if (activeFilter === 'they_owe') filtered = filtered.filter(d => !d.paid && d.type === 'they_owe');
    if (activeFilter === 'paid') filtered = filtered.filter(d => d.paid);
  ...
  function generatePersonSummaryText(person, filtered) {
    const cur = sym();
    let title = '';
    let balanceLabel = '';
    
    if (activeFilter === 'paid') {
      title = `*📑 Settled Dues Statement for ${person}*`;
      const iPaid = filtered.filter(d => d.type === 'i_owe').reduce((s, d) => s + d.amount, 0);
      const theyPaid = filtered.filter(d => d.type === 'they_owe').reduce((s, d) => s + d.amount, 0);
      balanceLabel = `*Total Settled:* I paid ${cur}${iPaid.toFixed(2)}, They paid ${cur}${theyPaid.toFixed(2)}`;
    } else {
      let filterName = 'Pending Dues';
      if (activeFilter === 'i_owe') filterName = 'Dues (I Owe)';
      if (activeFilter === 'they_owe') filterName = 'Dues (They Owe Me)';
      title = `*📑 ${filterName} Statement for ${person}*`;
      
      const total = filtered.reduce((s, d) => s + (d.type === 'they_owe' ? 1 : -1) * d.amount, 0);
      balanceLabel = `*Current Balance:* ${cur}${Math.abs(total).toFixed(2)} ${total >= 0 ? '(You owe me)' : '(I owe you)'}`;
    }

    const detail = filtered.map(d => {
      let typeStr = '';
      if (d.paid) {
        typeStr = d.type === 'i_owe' ? '[Settled - I Owed] ' : '[Settled - They Owed Me] ';
      } else {
        typeStr = d.type === 'i_owe' ? '[I Owe] ' : '[They Owe Me] ';
      }
      let itemStr = `• ${typeStr}${d.description || 'Due'}: ${cur}${d.amount.toFixed(2)} (${fmtDate(d.date)})`;
      if (d.paid && d.paidDate) {
        itemStr += ` (Settled: ${fmtDate(d.paidDate)})`;
      }
      return itemStr;
    }).join('\n');
  ...
  function sharePersonSummary(person) {
    const filtered = getFilteredDuesForPerson(person);
    if (filtered.length === 0) { window.MT.ui?.showToast('No matching dues to share', 'info'); return; }
    const text = generatePersonSummaryText(person, filtered);
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
  }

  function copyDueItem(id) {
    const list = loadDues();
    const d = list.find(x => x.id === id);
    if (!d) return;
    const cur = sym();
    ...
    navigator.clipboard.writeText(text).then(() => {
        window.MT.ui?.showToast('Copied to clipboard', 'success');
    }).catch(() => {
        window.MT.ui?.showToast('Failed to copy', 'danger');
    });
  }

  function copyPersonSummary(person) {
    const filtered = getFilteredDuesForPerson(person);
    if (filtered.length === 0) { window.MT.ui?.showToast('No matching dues to copy', 'info'); return; }
    const text = generatePersonSummaryText(person, filtered);
    navigator.clipboard.writeText(text).then(() => {
        window.MT.ui?.showToast('Copied to clipboard', 'success');
    }).catch(() => {
  ...
  window.MT.dues = {
    ...
    shareDueItem, sharePersonSummary, copyDueItem, copyPersonSummary,
