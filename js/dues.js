...
(function () {
  ...
  function sharePersonSummary(person) {
    ...
    const detail = pending.map(d => `• ${d.description || 'Due'}: ${cur}${d.amount} (${fmtDate(d.date)})`).join('\n');
  ...
  function copyPersonSummary(person) {
    const list = loadDues();
    const pending = list.filter(d => (d.person || '').trim().toLowerCase() === person.toLowerCase() && !d.paid);
    if (pending.length === 0) { window.MT.ui?.showToast('No pending dues to copy', 'info'); return; }
    const cur = sym();
    const total = pending.reduce((s, d) => s + (d.type === 'they_owe' ? 1 : -1) * d.amount, 0);
    const detail = pending.map(d => `• ${d.description || 'Due'}: ${cur}${d.amount} (${fmtDate(d.date)})`).join('\n');
    const text = `*📑 Statement for ${person}*\n\n*Current Balance:* ${cur}${Math.abs(total).toFixed(2)} ${total >= 0 ? '(You owe me)' : '(I owe you)'}\n\n*Pending Items:*\n${detail}\n\n_Generated by LUDARP Money Tracker_`;
